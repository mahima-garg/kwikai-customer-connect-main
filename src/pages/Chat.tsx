...
const botResponses = {
  ...
  refundStatus: (refundInfo?: any, orderNumber?: string) => {
    if (!refundInfo) {
  ...
  refundNotReceived: (refundInfo?: any, orderNumber?: string) => {
    if (!refundInfo) {
    ...
    if (refundInfo.status === "Success") {
      return `✅ Refund Status for Order #${orderNumber}: Success\n📌 ARN Number: ${refundInfo.arn_number || "Processing"}\n\nNote: It typically takes 3–5 working days for the bank to credit the amount.\n\nHas it been more than 7 days since this status update? If yes, please write to 📧 support@gokwik.co with your bank statement (from date of transaction till today), and we'll help you further.\n\nIs there anything else I can assist you with?`;
    } else {
      return `Your refund for Order #${orderNumber} is in process. You'll receive the amount within 3–5 working days. Thanks for your patience!\n\nFeel free to check back if you don't receive it within the expected timeframe.`;
    }
  },
  orderStatus: (order?: any) => {
    ...
    if (order.order_status === "confirmed" || order.order_status === "shipped" || order.order_status === "delivered") {
      ...
      return `Your order #${order.shopify_order_name} was CANCELLED, but your payment was successful.\n${hasRefund ? "A refund has already been initiated. You'll get the amount within 5–8 working days." : "A refund will be processed shortly."}\n\nWould you like me to check the refund status for you?`;
  ...
  adonc: (orderNumber?: string) => `Looks like your order #${orderNumber || ""} didn't go through, but your payment was successful. A refund will be processed automatically and you should receive it within 5–8 working days.\n\nWould you like me to track the refund status for you?`,
  cancelOrder: "To cancel your order, please contact the merchant brand directly.\nWe do not handle cancellations from our end.\n\nIs there something else I can help you with?",
  default: "I'm not sure how to help with that. Could you please be more specific? You can ask about refund status, order status, or payment issues.",
...
const Chat = () => {
  ...
  const handleARNInput = (text: string) => {
    ...
    return `✅ Refund Status for ARN ${text}:\nOrder: ${result.order.shopify_order_name}\nAmount: ₹${result.refund.amount}\nStatus: ${result.refund.status}\nRefund Date: ${new Date(result.refund.refunded_at || result.refund.created_at).toLocaleDateString()}\n\nIs there anything else you'd like to know about this refund?`;
  };
  ...
  const processUserMessage = (text: string) => {
    ...
    if (text.includes("refund status") || text.includes("where is my refund") || text.includes("check refund")) {
      ...
      setWaitingForOrderNumber(true);
      return botResponses.askOrderNumber;
    } else if (text.includes("amount debited") || text.includes("payment deducted") || text.includes("paid but no order")) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 container mx-auto px-4 py-6 flex flex-col max-w-2xl">
        ...
        <div className="flex-1 bg-white rounded-t-lg shadow overflow-hidden flex flex-col">
          ...
          {/* Messages container */}
          <div className="flex-1 p-4 overflow-y-auto space-y-4" style={{ height: "400px" }}>
          ...
          <div className="p-4 border-t bg-white">
            ...
            >
              <Input
                ...
                placeholder={waitingForOrderNumber ? "Enter your order number..." : 
                              waitingForARN ? "Enter your ARN number..." : 
                              "Type your message..."}
                className="flex-1"
                disabled={isTyping}
              />
