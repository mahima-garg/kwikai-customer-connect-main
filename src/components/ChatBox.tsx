...
const botResponses = {
  ...
  orderStatus: (order?: any) => {
    ...
    if (order.order_status === "confirmed") {
      ...
      const refundStatus = hasRefund ? order.refunds[0].status === "Success" ? 
        "Your refund has been processed and will be credited within 2-3 business days." :
        "Your refund is being processed." : "Your refund will be processed shortly.";
  ...
  default: "I'm not sure how to help with that. Could you please be more specific? You can ask about:\n- Refund Status\n- Order Status\n- Amount Debited but Order not Confirmed",
};
...
export function ChatBox({ onClose }: ChatBoxProps) {
  ...
  const findOrderByNumber = (orderNumber: string) => {
    ...
    return detailedOrders.find(order => 
  ...
  const processUserMessage = (text: string) => {
    ...
    if (normalizedText.includes("refund status") || normalizedText.includes("where is my refund") || normalizedText.includes("check refund")) {
      setCurrentContext("refund");
      setWaitingForOrderNumber(true);
      return botResponses.askOrderNumber;
    }
    
    if (normalizedText.includes("order status") || normalizedText.includes("my order") || normalizedText.includes("track")) {
      setCurrentContext("order");
      setWaitingForOrderNumber(true);
      return botResponses.askOrderNumber;
    }
    
    if (normalizedText.includes("amount debited") || normalizedText.includes("payment deducted") || normalizedText.includes("paid but no order")) {
